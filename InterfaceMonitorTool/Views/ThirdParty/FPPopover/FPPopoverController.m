...
-(id)initWithViewController:(id<FPPopoverControllerSource>)viewController
{
    self = [super init];
    if(self)
    {
        ...
        _touchView = [[FPTouchView alloc] initWithFrame:rect];
        ...
        _viewController = [(UIViewController*)viewController retain];
        self.contentSize = _viewController.view.bounds.size;//CGSizeMake(200, 300); //default size
        ...
        if ([viewController respondsToSelector:@selector(myTitleView)]) {
            UIView* titleView = [viewController myTitleView];
...
-(void)setupView
{
    self.view.frame = CGRectMake(0, 22, [self parentWidth], [self parentHeight]);
